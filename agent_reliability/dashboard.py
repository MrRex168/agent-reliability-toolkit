    ...
    def index():
        ...
        for index, record in enumerate(records):
            ...
            row_parts.append(
                ...
                f"<td><span class='status status-{status_class}'>{status}</span></td></tr>"
            )
        rows = "".join(row_parts)
        if latest:
            ...
            cards = (
                "<div class='grid'>"
                f"<div class='card metric-card'><div class='metric-label'>Latest reliability</div><div class='metric'>{latest.reliability_score:.1f}</div><div class='metric-change'>{reliability_change}</div></div>"
                f"<div class='card metric-card'><div class='metric-label'>Task success</div><div class='metric'>{latest.task_success:.1f}%</div><div class='metric-change'>{task_success_change}</div></div>"
                f"<div class='card metric-card'><div class='metric-label'>Consistency</div><div class='metric'>{latest.consistency:.1f}%</div><div class='metric-change'>{consistency_change}</div></div>"
                f"<div class='card metric-card'><div class='metric-label'>Failed runs</div><div class='metric'>{latest.failed_runs} / {latest.total_runs}</div><div class='metric-change'>{failed_change}</div></div>"
                "</div>"
            )
